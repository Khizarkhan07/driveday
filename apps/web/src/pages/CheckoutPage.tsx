...
export function CheckoutPage() {
  ...
  if (!stripePromise) {
    return (
      <Banner tone="danger">Payment is not configured. Please contact support.</Banner>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-ink-400 mt-1">Your payment is encrypted and secure</p>
      </div>

      {/* Order summary */}
      <div className="rounded-xl bg-ink-800/60 border border-ink-700/50 p-4 flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold text-ink-500 uppercase tracking-wider">Vehicle</p>
          <p className="text-white font-semibold">
            {vehicle.registration} · {[vehicle.make, vehicle.model].filter(Boolean).join(" ")}
          </p>
          <p className="text-xs text-ink-400 mt-0.5">1-day policy · {driver.firstName} {driver.lastName}</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-ink-500">Total</p>
          <p className="text-white font-extrabold text-lg">
            {/* will be filled by stripe */}
          </p>
        </div>
      </div>
            ...
            options={{
              ...
              appearance: {
                ...
                variables: {
                  ...
                  colorText: "#ffffff",
                  colorTextSecondary: "#9598ab",
                  colorTextPlaceholder: "#4a4d63",
                  borderRadius: "12px",
                },
              },
            }}
      ...
      <div className="flex items-center justify-between text-xs text-ink-600">
        ...
        <button className="hover:text-ink-400 transition" onClick={() => navigate(-1)}>
          ← Go back
        </button>
      </div>
    </div>
