...
export function AdminPoliciesPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-extrabold text-white">Policies</h1>
      ...
      <Card className="p-0 overflow-hidden">
        {isLoading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {data?.policies.map((p) => (
                <tr key={p.id} className="border-b border-ink-800/40 hover:bg-ink-800/40 transition last:border-0">
                  ...
                  <td className="px-5 py-3">
                    <Link to={`/admin/users/${p.user.id}`} className="hover:text-brand-400 transition">
                      <p className="text-ink-300">{p.user.firstName} {p.user.lastName}</p>
                      <p className="text-ink-500 text-xs">{p.user.email}</p>
                    </Link>
                  </td>
                  <td className="px-5 py-3 text-ink-300">
                    <p className="font-medium">{p.quote.vehicle.registration}</p>
                    <p className="text-xs text-ink-500">{p.quote.vehicle.make} {p.quote.vehicle.model}</p>
                  </td>
                  <td className="px-5 py-3">
                    <Badge tone={statusTone(p.status)}>{p.status}</Badge>
                  </td>
                  <td className="px-5 py-3 text-white font-semibold">{money(p.quote.totalPence)}</td>
                  <td className="px-5 py-3 text-ink-400 text-xs">
                    <p>{fmtDate(p.startDate)}</p>
                    <p>{fmtDate(p.endDate)}</p>
                  </td>
                  <td className="px-5 py-3 text-ink-400 text-xs">{p._count.documents}</td>
                </tr>
              ))}
              {data?.policies.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-5 py-8 text-center text-ink-500">No policies found</td>
                </tr>
              )}
            </tbody>
          </table>
