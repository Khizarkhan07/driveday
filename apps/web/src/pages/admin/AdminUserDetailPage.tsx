...
export function AdminUserDetailPage() {
  ...
  if (!data) return <p className="text-red-500 text-sm">User not found</p>;

  const { user, events } = data;

  return (
    <div className="space-y-6">
      <Link to="/admin/users" className="text-ink/55 hover:text-ink text-sm transition">← Users</Link>
      ...
      <Card>
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-ink/55 text-sm mt-0.5">{user.email}</p>
            <p className="text-ink/35 text-xs mt-1 font-mono">ID: {user.id}</p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <Badge tone={user.role === "ADMIN" ? "warning" : "neutral"}>{user.role}</Badge>
            <span className="text-xs text-ink/40">Joined {fmtDate(user.createdAt)}</span>
          </div>
        </div>
      </Card>

      <div>
        <h2 className="text-base font-semibold text-ink mb-3">Policies ({user.policies.length})</h2>
        {user.policies.length === 0 ? (
          <Card><p className="text-ink/45 text-sm text-center py-4">No policies</p></Card>
        ) : (
              ...
              <Card key={p.id} className="p-4 sm:p-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-ink font-bold">{money(p.quote.totalPence)}</p>
                    <p className="text-ink/40 text-xs mt-1">{p.documents.length} docs</p>
                  </div>
                </div>
              </Card>
            ))}
      ...
      <div>
        <h2 className="text-base font-semibold text-ink mb-3">Event log ({events.length})</h2>
